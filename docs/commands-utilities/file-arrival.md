...
### Support for Debugging File Arrival Job Failures

This advice is addressed primarily to the Continuous internal technical support staff.  However, clients who have discovered the program dump report identified in this section should kindly include the dump report label value as they submit a Support request.

LSAM PTF # 211191 added a new capability to both the CHKFILE and CHKIFSFIL commands.  A program internal field called AbortLoc (meaning "abort location") is now being set whenever a program logic failure has been detected, and the program will then be forced to end abnormally by the the program's SrAbort subroutine.  In cases of severe program failure, the RPG default failure management subroutine called *PSSR will generate a program dump report.  Although the standard for the IBM i Agent programs is to have visibility (of program data content) suppressed, the program dump report will be labelled at the top of each report page with the value of the AbortLoc field.  This value will point the Continuous level 3 Support staff (that is, the Engineering team who have access to program source code) to the exact location in the program source code where the program failure was first detected.  That is, the AbortLoc value marks which program location called the SrAbort subroutine.

### $@ Agent Variable Names Support Command Keywords

As the OpCon general standards for File Arrival jobs were evolving, a work-around method was required to engage the full capabilities that are unique to the IBM i Agent File Arrival commands.  The temporary solution was to set any command keyword value from the Variables Tab of an OpCon job if it was not already supported by the IBM i File Arrival job, or not entered as a command parameter when an IBM i Batch Job was executing the CHKFILE or the CHKIFSFIL command. This made it possible to engage any of this Agent's capabilities that are not already supported by data entry field in the OpCon User Interface job definition panel.
...
:::important
Starting with Agent version 21.1.097 the following File Arrival job (command) parameters can be entered in the main OpCon Solution Manager job master record definition panel, **eliminating the need to use the $@Variables** for most uses of the File Arrival job.  For this method of configuring File Arrival jobs, or when an IBM i job master is a Batch Job that specifies either the CHKFILE or CHKIFSFIL command, do NOT add any $@Variables to the job master VARIABLES tab.
...
IBM i Agent users could have File Arrival job master records previously configured in the OpCon server that are using the $@Variables to define one or more of these long-established File Arrival command parameters.  For these cases, see the **WARNING** just below.  After upgrading older versions of the IBM i Agent, users should remove the matching $@Variables and instead register the values for those variables in the matching data entry fields that are supported by Solution Manager data entry.  The same instruction applies to IBM i Batch Jobs that had been configured to execute from the Call Command box the commands CHKFILE or CHKIFSFIL.  If those jobs already specify the command parameters listed above, and these parameters were registered as $@Variables under the OpCon Job Master VARIABLES tab, this creates a conflict that typically results in the parameter keywords appearing in the final job's command line with blank values, producing unexpected results or even a job failure.

If there are still a few, less frequently used parameters of the CHKFILE and CHKIFSFIL commands that are not supported by Solution Manager data entry, those parameters can still be supported by $@Variables under the job master VARIABLES tab.
:::

#### Methods for using $@Variables

:::warning
Do not use $@Variables under the OpCon job master VARIABLES tab under the following circumstances:
 - Do NOT use $@Variables when the File Arrival job type has been selected in the Solution Manager User Interface for IBM i job master record maintenance.
 - Do NOT use $@Variables with the CHK* commands entered in the Call Command box for an IBM i “Batch job” job type

When either of these Rules is not observed, the presence of $@Vaiables in the VARIABLES tab will cause unexpected results in the command parameters that the IBM i Agent assembles for the SBMJOB command that starts jobs using the LSAM commands CHKFILE of CHKIFSFIL.  The usual unsatisfactory result is that these parameter values will be included in the command line with a blank value, disabling the intention of the command parameters.
:::
