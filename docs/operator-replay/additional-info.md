...
| RESET  | This special function instructs the IBM i display manager to reset from an error condition that has previously set the input-inhibited indicator along with the screen error indicator (a low-level 5250 device status instruction). This function key has no effect and does not generate an error if the screen is not in a reset able condition. It should only be issued when the appropriate circumstances have been detected. Reset has no effect if the input-inhibited indicator has been set during the wait period after <**Enter**> or another function key has been used to return control of the screen to the IBM i display manager (and/or the program that presented the screen). See additional details in the following discussion about  |

### LSAM PTF # 211164: Improved RESET key response to BELL signal (Input Inhibited)

The RESET keyboard key is used to clear the display from an input inhibited state and allow the keyboard to resume allowed data entry in input-capable fields (at the correct display position).
                                                                       
Previous versions of the LSAM Operator Replay Script driver could send the RESET keyboard function if it was specified on a Script Step after the Step that could cause an invalid data entry value or position. The emulation of a VTxxx display device does not normally get a reply from the IBM i Workstation Manager after a local display RESET is executed. This was causing the Operator Replay Script driver to timeout after waiting for a response to the last Function to Send (which was the RESET function in this case).                                          

Although a BELL signal reporting an Input Inhibited status is used to report that there is some error in the Operator Replay String to Send data, and/or that the display format has changed since the Script was originally designed.  Either way, the BELL signal usually implies that the Script cannot continue reliably, so it was considered acceptable that the Script should be forced to fail, either because of the SMA0105 error (BELL signal received) or because of SMA0106 = timeout while waiting for the IBM i host to respond to the RESET function key.       
                                                                       
:::note 
The usual response to a BELL signal has been to post error code SMA0105.  But trying to use the RESET function code in the Step immediately after the Step that produced a BELL signal was overriding that error with error SMA0106 because the Host was not responding when the RESET function was sent from the Script driver.
:::

The enhancement delivered by this PTF has removed the hard-coded response to a BELL signal and some hard coded logic overrides imposed after the RESET function key is sent.  The new program logic allows the Script driver program to process the Sript Step in the same way as any other Script Step.  This change should, in most cases, remain compatible with existing Scripts that had included the RESET function key.  But the importance of this change is that a Script Step that specifies RESET as the Function to Send is now allowed to utilized any of the other features of a Script Step.
                                                                       
Consider these options that can be used to improve how a RESET function might recover from a soft BELL error that does not require the Script to fail.  Or, it can be used to assure that the Script will fail in an orderly fashion instead of the immediate forced failure.

 - The Top and Bottom control strings can be used to test whether the screen display will be managed by the RESET function key, or whether this Step can be skipped because of certain soft errors that do not affect the outcome of the Scripted workstation function.  As normal, failure to match either control string rule will prevent the RESET Step from being processed, and the Script will continue with the next sequential Step.
                                                                       
 - The Data Capture and Response Rules can be used to perform tests that determine if the Script must branch to an orderly failure, or to some other location within the same or a different Script, in response to the BELL character.  This is the most significant improvement to the RESET Step processing, because previously the LSAM Administrator had no control over a forced immediate failure of a Script that sent the SMA0105 error, or if the SMA0106 error was generated by a RESET function in a later Step.  If the Response Rules will store a Script or Step name value in a Dynamic Variable {TOKEN}, the tokens can be stored in either of the Step Branching control fields.
                                                                       
Due to the relative complexity of job steps that may frequently involve a BELL error, it is usually necessary to set up proposed Script Steps for execution in a test environment.  The Operator Replay log of the screen displays and keyboard actions can be used to observe the behavior of the proposed Script Steps, as long as the test Script imitates the behavior of the production Script.
                                                                       
:::tip
To discover useful Operator Replay Script Step sequences, create a Script for test purposes that forces the appearance of a BELL signal. One way to force the appearance of a BELL signal is to define a Script that navigates to an IBM i command entry display.  Then use the Operator Replay Step's cursor movement codes to move the cursor to a location on the display that does not accept input, and then follow the cursor movements with a keyboard text character.
:::
...
```
 ...
 LSAVARR7                  Display Dynamic Variable DTAARA Access     00/00/00  
 USERNAME                                                             16:43:27  
...
Control strings         If no match: F S=Skip, F=Fail   Comp numeric:   Y/N  
  Top     Compare rule: EQ  Blank,EQ,NE,GT,LT,GE,LT,=,<>,>,<,>=,<=           
                 Value: #                                                   +
                   Row:  1  Column:  1  Length:   1                          
```
